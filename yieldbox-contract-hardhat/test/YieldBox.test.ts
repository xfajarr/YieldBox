...
describe("YieldBox Contract Comprehensive Tests", async function () {
  ...
  it("Should deploy all contracts with correct configurations", async function () {
    ...
    assert.equal((await openCrateNFT.read.factory()).toLowerCase(), (openCrateFactory.address as string).toLowerCase());

    assert.equal(await strategyRegistry.read.strategyCount(), 1n);
    const strategy = await strategyRegistry.read.strategy([1n]);
    assert.equal(strategy.adapter.toLowerCase(), mockYieldAdapter.address.toLowerCase());
  ...
  it("Should allow owner to register new strategies", async function () {
    const txHash = await strategyRegistry.write.registerStrategy([mockYieldAdapter.address, "0x1234", 1, true], {
    ...
    const event = receipt.logs.find((log) => {
      try {
        ...
        return decoded.eventName === "StrategyRegistered";
      } catch {
        return false;
      }
    });
    ...
    assert.equal(decodedEvent.args.strategyId, 2n);
    assert.equal((decodedEvent.args.adapter as string).toLowerCase(), mockYieldAdapter.address.toLowerCase());
    assert.equal(decodedEvent.args.riskLevel, 1);

    assert.equal(await strategyRegistry.read.strategyCount(), 2n);
    
    const newStrategy = await strategyRegistry.read.strategy([2n]);
    assert.equal(newStrategy.adapter.toLowerCase(), mockYieldAdapter.address.toLowerCase());
  ...
  it("Should successfully mint a crate with valid parameters", async function () {
    ...
    assert.equal(decodedEvent.args.tokenId, 1n);
    assert.equal((decodedEvent.args.owner as string).toLowerCase(), user1.account.address.toLowerCase());

    assert.equal((await openCrateNFT.read.ownerOf([1n])).toLowerCase(), user1.account.address.toLowerCase());
  ...
  it("Should create ERC6551 account for each minted crate", async function () {
    ...
    // Verify that account is correctly configured
    ...
    assert.equal(chainId.toString(), "31337");
    assert.equal(tokenContract.toLowerCase(), openCrateNFT.address.toLowerCase());
    assert.equal(tokenId, 2n);
    assert.equal((await accountContract.read.owner()).toLowerCase(), user1.account.address.toLowerCase());
  });
  ...
  it("Should correctly predict account addresses", async function () {
    ...
    assert.equal((deployment.account as string).toLowerCase(), predictedAddress.toLowerCase());
  });
  ...
  it("Should reject minting with inactive strategy", async function () {
    ...
    const invalidParams = { ...defaultMintParams };
  ...
  it("Should reject minting with risk level mismatch", async function () {
    // First activate strategy 2 with risk level 1
    await strategyRegistry.write.registerStrategy([mockYieldAdapter.address, "0x", 1, true], {
      account: owner.account.address,
    });
    
    const invalidParams = { ...defaultMintParams, riskLevel: 1, strategyId: 1n }; // RISK_MEDIUM with strategy 1 (RISK_HIGH)
  ...
  it("Should reject minting with invalid price", async function () {
    // Reactivate strategy 1 first
    await strategyRegistry.write.setStrategyStatus([1n, true], {
      account: owner.account.address,
    });
  ...
  it("Should update positions with Uniswap V4 data", async function () {
    ...
    const txHash = await openCrateNFT.write.updatePositions([1n, UNISWAP_V4_POSITIONS], {
        account: user1.account.address,
      });
    ...
    const event = receipt.logs.find((log) => {
      try {
        const decoded = decodeEventLog({
          abi: openCrateNFT.abi,
          eventName: "PositionsUpdated",
          data: log.data,
          topics: log.topics,
        });
        return decoded.eventName === "PositionsUpdated";
      } catch {
        return false;
      }
    });
    ...
    assert.equal(decodedEvent.args.tokenId, 1n);
    assert.equal(decodedEvent.args.positionCount, BigInt(UNISWAP_V4_POSITIONS.length));
    assert.equal(decodedEvent.args.updater.toLowerCase(), user1.account.address.toLowerCase());
    ...
    for (let i = 0; i < UNISWAP_V4_POSITIONS.length; i++) {
      ...
      assert.equal(positions[i].netApyBps, BigInt(UNISWAP_V4_POSITIONS[i].netApyBps));
  ...
  it("Should correctly calculate position count", async function () {
    ...
    assert.equal(await openCrateNFT.read.positionsCount([1n]), 2n);
  ...
  it("Should update crate price", async function () {
    ...
    const txHash = await openCrateNFT.write.updatePrice([1n, newPrice], {
        account: user1.account.address,
      });
    ...
    const event = receipt.logs.find((log) => {
      try {
        ...
        return decoded.eventName === "PriceUpdated";
      } catch {
        return false;
      }
    });
    ...
    assert.equal(decodedEvent.args.tokenId, 1n);
    assert.equal(decodedEvent.args.priceUsd, newPrice);
  ...
  it("Should set boost multiplier", async function () {
    ...
    const txHash = await openCrateNFT.write.setBoostMultiplier([1n, newBoost], {
        account: user1.account.address,
      });
    ...
    const event = receipt.logs.find((log) => {
      try {
        const decoded = decodeEventLog({
          abi: openCrateNFT.abi,
          eventName: "BoostUpdated",
          data: log.data,
          topics: log.topics,
        });
        return decoded.eventName === "BoostUpdated";
      } catch {
        return false;
      }
    });
    ...
    assert.equal(decodedEvent.args.tokenId, 1n);
    assert.equal(decodedEvent.args.boostMultiplierBps, newBoost);
  ...
  it("Should extend lock duration", async function () {
    ...
    const txHash = await openCrateNFT.write.extendLock([1n, additionalDuration], {
        account: user1.account.address,
      });
    ...
    const event = receipt.logs.find((log) => {
      try {
        ...
        return decoded.eventName === "LockExtended";
      } catch {
        return false;
      }
    });
  ...
  it("Should handle complete crate lifecycle with multiple protocols", async function () {
    ...
    assert.equal(await openCrateNFT.read.totalMinted(), 33n);
